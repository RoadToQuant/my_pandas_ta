...
def tsi(close, fast=None, slow=None, signal=None, scalar=None, mamode=None, drift=None, offset=None, **kwargs):
    ...
    slow_ema = ma(mamode, diff, length=slow, **kwargs)
    fast_slow_ema = ma(mamode, slow_ema, length=fast, **kwargs)

    abs_diff = diff.abs()
    abs_slow_ema = ma(mamode, abs_diff, length=slow, **kwargs)
    abs_fast_slow_ema = ma(mamode, abs_slow_ema, length=fast, **kwargs)
